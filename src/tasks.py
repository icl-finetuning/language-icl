...
def mean_l2_error(ys_pred, ys):
    # Print max values in ys and ys_pred
    max_value_ys = torch.max(ys)
    max_value_ys_pred = torch.max(ys_pred)
    print(f"Max value in ys: {max_value_ys.item()}")
    print(f"Max value in ys_pred: {max_value_ys_pred.item()}")
...
class RecursiveRelu2nn(SlidingWindowSequentialTasks):
    ...
    # def generate_sequence(self, x0, sequence_length):
    #     """
    #     Generate a sequence starting at x0 of length sequence_length using the provided functions and sliding window, without normalization.
        
    #     Parameters:
    #     x0 : Tensor (self.batch_size, self.n_dims)
    #         Initial values for the sequence.
    #     sequence_length : int
    #         Length of the sequence to be generated.
        
    #     Returns:
    #     xs : Tensor (self.batch_size, sequence_length, self.n_dims)
    #         Generated sequences.
    #     ys : Tensor (self.batch_size, sequence_length, self.n_dims)
    ...
    #     # Generate sequence
    #     for t in range(1, sequence_length):
    #         # Sum the function applications
    #         x = torch.sum(torch.stack([f(xs[:, t - (i+1), :]) if t - (i+1) >= 0 else torch.zeros_like(xs[:, 0, :]) for i, f in enumerate(self.functions)]), dim=0)
            
    #         # Debugging: Check for NaN or infinity
    #         if torch.isnan(x).any() or torch.isinf(x).any():
    #             print(f"Invalid value detected at step {t}: x = {x}")
    #             # raise ValueError(f"Invalid value detected at step {t}")
            
    #         # Update xs and ys without normalization
    #         xs[:, t, :] = x
    #         ys[:, t - 1, :] = xs[:, t, :]
            
    #         # Print max value in xs
    #         max_value_xs = torch.max(xs[:, t, :])
    #         print(f"Max value in xs at step {t}: {max_value_xs.item()}")
            
    #         # Print max value in ys
    #         if t > 1:  # Print ys max value after the first step
    #             max_value_ys = torch.max(ys[:, t - 1, :])
    #             print(f"Max value in ys at step {t}: {max_value_ys.item()}")
        
    #     # Compute the final ys element
    #     next_x = torch.sum(torch.stack([f(xs[:, sequence_length - (i+1), :]) if sequence_length - (i+1) >= 0 else torch.zeros_like(xs[:, 0, :]) for i, f in enumerate(self.functions)]), dim=0)
        
    #     # Debugging: Check for NaN or infinity
    #     if torch.isnan(next_x).any() or torch.isinf(next_x).any():
    #         print(f"Invalid value detected at the final step: next_x = {next_x}")
    #         # raise ValueError("Invalid value detected at the final step")
        
    #     ys[:, -1, :] = next_x

    #     # Print final max value in xs and ys
    #     final_max_value_xs = torch.max(xs)
    #     final_max_value_ys = torch.max(ys)
    #     print(f"Final max value in xs: {final_max_value_xs.item()}")
    #     print(f"Final max value in ys: {final_max_value_ys.item()}")
