...
def eval_batch(model, task_sampler, xs, xs_p=None, seq=False, n_positions=101):
    ...
    model_name = model.module.name if isinstance(model, torch.nn.DataParallel) else model.name

    if torch.cuda.is_available() and model_name.split("_")[0] in ["gpt2", "lstm"]:
        device = "cuda"
    else:
        device = "cpu"
...
def compute_evals(all_models, evaluation_kwargs, save_path=None, recompute=False, random_ys=False, seq=False, n_positions=101):
    ...
    for eval_name, kwargs in tqdm(evaluation_kwargs.items()):
        ...
        for model in all_models:
            model_name = model.module.name if isinstance(model, torch.nn.DataParallel) else model.name
            if model_name in metrics and not recompute:
                continue
            # this is where the evaluation happens
            # metrics[model.name] = eval_model(model, **kwargs, random_ys)
            metrics[model_name] = eval_model(model, seq=seq, n_positions=n_positions, **kwargs)
...
def read_run_dir(run_dir):
    ...
    for task in os.listdir(run_dir):
        ...
        for run_id in os.listdir(task_dir):
            if "gpt2" in run_id or "pretrained" in run_id:
                continue
